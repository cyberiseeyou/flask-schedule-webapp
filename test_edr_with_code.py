"""Direct test of EDR functionality with MFA code as argument"""
import sys
import os
...
if len(sys.argv) < 2:
    print("Usage: python test_edr_with_code.py <MFA_CODE>")
    print("Please run the script and provide the MFA code you receive")
    sys.exit(1)

mfa_code = sys.argv[1]

# Event IDs to test (provided by user)
event_ids = ["606878", "604207", "607022"]
    ...
    if edr_gen.complete_authentication_with_mfa_code(mfa_code):
        ...
        for event_id in event_ids:
            ...
            if edr_data:
                ...
                if item_details:
                    print(f"\n    Sample Items:")
                    for i, item in enumerate(item_details[:5], 1):
